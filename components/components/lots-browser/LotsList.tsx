      ...
      {lots.map((lot) => (
        <div key={lot.id} className="border rounded-xl p-4 shadow-md bg-white">
          ...
          <p className="mt-2 text-sm">
            <strong>Estimation :</strong> {lot.estimated_price_eur.min.toLocaleString()} – {lot.estimated_price_eur.max.toLocaleString()} €
          </p>
          <p className="text-xs text-gray-500">
            {new Date(lot.auction_date).toLocaleString('fr-FR')}
          </p>
